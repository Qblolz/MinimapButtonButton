...
local function getLibDataBroker ()
  return _G.LibStub and _G.LibStub:GetLibrary('LibDataBroker-1.1', true);
end
...
local function collectLibDBIconButtons ()
  ...
  local buttons = LibDBIcon.objects
...
local function initMainButton ()
  mainButton = _G.CreateFrame('Button', addonName .. 'Button', _G.UIParent, 'UIPanelButtonTemplate');
  mainButton:SetParent(_G.UIParent);
  --mainButton:SetFrameStrata(Constants.FRAME_STRATA);
  mainButton:SetFrameLevel(4)
...
local function hookLibDBIconButtons ()
  local LibDataBroker = getLibDataBroker();

  if (not LibDataBroker) then
    return;
  end

  if LibDataBroker.RegisterCallback then
    LibDataBroker.RegisterCallback(addonName, 'LibDBIcon_IconCreated', function (_, button)
...
addon.import('Core/Events').registerEvent('PLAYER_ENTERING_WORLD', function ()
  --[[ executing on next frame to wait for addons that create minimap buttons
       on PLAYER_LOGIN ]]
  C_Timer:After(0, function() init() end)
  C_Timer:After(1, function() collectMinimapButtonsAndUpdateLayout() end)
